...
class ResNet1D(nn.Module):
    ...
    def __init__(self, in_channels, base_filters, kernel_size, stride, groups, n_block, n_classes, downsample_gap=2, increasefilter_gap=4, use_bn=True, use_do=True, verbose=False, classifier=None):
        super(ResNet1D, self).__init__()
        
        if classifier is None:
            classifier = linear_classifier(base_filters * 2**(n_block // increasefilter_gap - 1), n_classes)
        ...
        ##self.features = nn.Linear(out_channels, 32)
        ##self.f_relu = nn.ReLU()
        #self.f_drop = nn.Dropout(p=0.5)
...
class RNN_feature_extractor_classifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # Take mean of bidirectional layers and concatenate them
